...
export async function POST(req: Request) {
  try {
    ...
    const { name, level, isSoftSkill, description } = await req.json();
    ...
    const insertResult = await query(
      `INSERT INTO skill (cv_id, name, skill_level, is_soft_skill, description)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING skill_id`,
      [cvId, name, level, isSoftSkill, description || null]
    );

    return NextResponse.json({
      success: true,
      skillId: insertResult.rows[0].skill_id,
    });
  } catch (error) {
